...
export type MeasurementStatus = 'GREEN' | 'YELLOW' | 'ORANGE' | 'RED';
 ...
 *  < 17          → RED    (Magreza Grave)
 *  17 – 18.49    → ORANGE (Magreza Leve)
 *  18.5 – 24.99  → GREEN  (Peso Normal / Ideal)
 *  25 – 29.99    → YELLOW (Sobrepeso)
 *  30 – 34.99    → ORANGE (Obesidade Grau I)
 *  35 – 39.99    → RED    (Obesidade Grau II - Severa)
 *  ≥ 40          → RED    (Obesidade Grau III - Mórbida)
 */
export const getBMIStatus = (bmi: number): MeasurementStatus => {
  if (bmi < 17)    return 'RED';
...
export const getBMILabel = (bmi: number): string => {
  if (bmi < 17)    return 'Magreza Grave';
  if (bmi < 18.5)  return 'Magreza Leve';
  if (bmi < 25)    return 'Peso Normal (Ideal)';
  if (bmi < 30)    return 'Sobrepeso';
  if (bmi < 35)    return 'Obesidade Grau I';
  if (bmi < 40)    return 'Obesidade Grau II (Severa)';
  return 'Obesidade Grau III (Mórbida)';
};

export const getBMIRiskLabel = (bmi: number): string => {
  if (bmi < 17)    return 'Risco Elevado';
  if (bmi < 18.5)  return 'Risco Elevado';
  if (bmi < 25)    return 'Baixo / Normal';
  if (bmi < 30)    return 'Aumentado';
  if (bmi < 35)    return 'Alto';
  if (bmi < 40)    return 'Muito Alto';
  return 'Altíssimo';
};
...
/**
 * Returns the best available display color for a stored measurement.
 * Priority: stored status → auto-computed → reference scale → null.
 */
export const getMeasurementDisplayStatus = (
  type: MeasurementType,
  value: number,
  storedStatus?: string | null,
): MeasurementStatus | null => {
  if (storedStatus && storedStatus in MEASUREMENT_STATUS_COLORS) {
    return storedStatus as MeasurementStatus;
  }
  return getAutoStatus(type, value) ?? getReferenceStatus(type, value);
};
...
export const getStatusIcon = (status: MeasurementStatus): string => {
  switch (status) {
    ...
    case 'ORANGE': return 'error-outline';
