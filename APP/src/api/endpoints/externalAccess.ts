...
export const externalAccessApi = {
  ...
  updatePathology: async (token: string, pathologyId: number, body: any) => {
    const url = `/api/external-access/${token}/pathologies/${pathologyId}`;
    return apiPublic.post(url, body).then(r => r.data);
  },
  ...
  addExternalWoundTracking: (token: string, formData: FormData): Promise<ApiResponse<ExternalElderlyWound>> => {
    const url = `/api/external-access/${token}/wound-trackings`;
